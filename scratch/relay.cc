...
NS_LOG_COMPONENT_DEFINE ("TestbedRelay");
...
{
  LogComponentEnable("TestbedRelay", LOG_LEVEL_INFO);
  GlobalValue::Bind ("SimulatorImplementationType", StringValue ("ns3::RealtimeSimulatorImpl"));
  GlobalValue::Bind ("ChecksumEnabled", BooleanValue (true));

  int          m_run              = 0;
  double       m_totalTime        = 2000.0;
  std::string  m_routing          = "split";
  bool         m_pcap             = false;
  double       m_charge           = 150.0; //6300.0; for 4 x AA
  ...
  emu.EnablePcap (ipAddr + "_" + m_routing + "_relay_" + std::to_string(m_run), device);
