...
///
/// \brief Implementation of SPLIT2 agent and related classes.
///
/// This is the main file of this software because %SPLIT2's behaviour is
/// implemented here.
///
...
#include "split2-routing-protocol.h"
...
///
/// We only use this value in order to define SPLIT2_NEIGHB_HOLD_TIME.
///
#define SPLIT2_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define SPLIT2_NEIGHB_HOLD_TIME   Time (3 * SPLIT2_REFRESH_INTERVAL)
/// Top holding time.
#define SPLIT2_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define SPLIT2_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define SPLIT2_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define SPLIT2_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define SPLIT2_UNSPEC_LINK        0
/// Asymmetric link type.
#define SPLIT2_ASYM_LINK          1
/// Symmetric link type.
#define SPLIT2_SYM_LINK           2
/// Lost link type.
#define SPLIT2_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define SPLIT2_NOT_NEIGH          0
/// Symmetric neighbor type.
#define SPLIT2_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define SPLIT2_MPR_NEIGH          2


/********** Willingness **********/

/// Willingness for forwarding packets from other nodes: never.
#define SPLIT2_WILL_NEVER         0
/// Willingness for forwarding packets from other nodes: low.
#define SPLIT2_WILL_LOW           1
/// Willingness for forwarding packets from other nodes: medium.
#define SPLIT2_WILL_DEFAULT       3
/// Willingness for forwarding packets from other nodes: high.
#define SPLIT2_WILL_HIGH          6
/// Willingness for forwarding packets from other nodes: always.
#define SPLIT2_WILL_ALWAYS        7


/********** Miscellaneous constants **********/

/// Maximum allowed jitter.
#define SPLIT2_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define SPLIT2_MAX_SEQ_NUM        65535
/// Random number between [0-SPLIT2_MAXJITTER] used to jitter SPLIT2 packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, SPLIT2_MAXJITTER)))


#define SPLIT2_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define SPLIT2_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define SPLIT2_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define SPLIT2_MAX_ADDRS          64
...
NS_LOG_COMPONENT_DEFINE ("Split2RoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::split2::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   ...
                   MakeEnumChecker (SPLIT2_WILL_NEVER, "never",
                                    SPLIT2_WILL_LOW, "low",
                                    SPLIT2_WILL_DEFAULT, "default",
                                    SPLIT2_WILL_HIGH, "high",
                                    SPLIT2_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive SPLIT2 packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::split2::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send SPLIT2 packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace),
                     "ns3::split2::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The SPLIT2 routing table has changed.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_routingTableChanged),
                     "ns3::split2::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created split2::RoutingProtocol");
  ...
  m_packetSequenceNumber = SPLIT2_MAX_SEQ_NUM;
  m_messageSequenceNumber = SPLIT2_MAX_SEQ_NUM;
  m_ansn = SPLIT2_MAX_SEQ_NUM;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting SPLIT2 on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunSplit2 = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), SPLIT2_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvSplit2,  this));
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() SPLIT2 socket");
        }
  ...
  if (canRunSplit2)
    {
      ...
      NS_LOG_DEBUG ("SPLIT2 on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %SPLIT2 packet following \RFC{3626} specification.
void
RoutingProtocol::RecvSplit2 (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("SPLIT2 node " << m_mainAddress << " received a SPLIT2 packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == SPLIT2_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  split2::PacketHeader split2PacketHeader;
  packet->RemoveHeader (split2PacketHeader);
  NS_ASSERT (split2PacketHeader.GetPacketLength () >= split2PacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = split2PacketHeader.GetPacketLength () - split2PacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Split2 Msg received with type "
  ...
  m_rxPacketTrace (split2PacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case split2::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SPLIT2 node " << m_mainAddress
            ...
            case split2::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SPLIT2 node " << m_mainAddress
            ...
            default:
              NS_LOG_DEBUG ("SPLIT2 message type " <<
        ...
        {
          NS_LOG_DEBUG ("SPLIT2 message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != split2::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all SPLIT2 messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == SPLIT2_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == SPLIT2_WILL_ALWAYS)
        {
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != SPLIT2_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
...
void
RoutingProtocol::ProcessHello (const split2::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const split2::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Link Set for SPLIT2 Node " << m_mainAddress);
    for (LinkSet::const_iterator link = links.begin (); link != links.end (); link++)
      {
        NS_LOG_DEBUG (*link);
      }
    NS_LOG_DEBUG ("** END dump Link Set for SPLIT2 Node " << m_mainAddress);

    const NeighborSet &neighbors = m_state.GetNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Neighbor Set for SPLIT2 Node " << m_mainAddress);
    for (NeighborSet::const_iterator neighbor = neighbors.begin (); neighbor != neighbors.end (); neighbor++)
      {
        NS_LOG_DEBUG (*neighbor);
      }
    NS_LOG_DEBUG ("** END dump Neighbor Set for SPLIT2 Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for SPLIT2 Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for SPLIT2 Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
void
RoutingProtocol::ProcessTc (const split2::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const split2::MessageHeader::Tc &tc = msg.GetTc ();
...
//      const Ipv4Address &addr = *i;
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for SPLIT2 Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for SPLIT2 Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}

void
RoutingProtocol::ProcessMid (const split2::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const split2::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const split2::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const split2::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<split2::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (split2::MessageHeader split2Message,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << split2Message.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (split2Message.GetTimeToLive () > 1)
    {
      const MprSelectorTuple *mprselTuple =
        m_state.FindMprSelectorTuple (GetMainAddress (senderAddress));
      if (mprselTuple != NULL)
        {
          split2Message.SetTimeToLive (split2Message.GetTimeToLive () - 1);
          split2Message.SetHopCount (split2Message.GetHopCount () + 1);
          // We have to introduce a random delay to avoid
          // synchronization with neighbors.
          QueueMessage (split2Message, JITTER);
          retransmitted = true;
        }
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + SPLIT2_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = split2Message.GetOriginatorAddress ();
      newDup.sequenceNumber = split2Message.GetMessageSequenceNumber ();
      newDup.expirationTime = now + SPLIT2_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (SPLIT2_DUP_HOLD_TIME,
...
void
RoutingProtocol::QueueMessage (const split2::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("SPLIT2 node " << m_mainAddress << " sending a SPLIT2 packet");

  // Add a header
  split2::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      i->first->SendTo (packet, 0, InetSocketAddress (bcast, SPLIT2_PORT_NUMBER));
    }
}
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("Split2 node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<split2::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
       ...
       message++)
    {
      ...
      if (++numMessages == SPLIT2_MAX_MSGS)
        {
...
RoutingProtocol::SendHello ()
{
  ...
  split2::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (SPLIT2_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  split2::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<split2::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      split2::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("SPLIT2 HELLO message size: " << int (msg.GetSerializedSize ())
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (SPLIT2_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  split2::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetVTime (SPLIT2_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (SPLIT2_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  split2::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<split2::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      split2::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-split2 interface
          if (UsesNonSplit2OutgoingInterface (route))
            {
  ...
  // add the routes using non-split2 outgoing interfaces to the list
...
bool
RoutingProtocol::UsesNonSplit2OutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-SPLIT2 interface if a match is found
  // before reaching the end of the list of excluded interfaces
  return ci != m_interfaceExclusions.end ();
}

void
RoutingProtocol::LinkSensing (const split2::MessageHeader &msg,
                              const split2::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  ...
  for (std::vector<split2::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (lt)
        {
        case SPLIT2_UNSPEC_LINK:
          linkTypeName = "UNSPEC_LINK";
          break;
        case SPLIT2_ASYM_LINK:
          linkTypeName = "ASYM_LINK";
          break;
        case SPLIT2_SYM_LINK:
          linkTypeName = "SYM_LINK";
          break;
        case SPLIT2_LOST_LINK:
      ...
      switch (nt)
        {
        case SPLIT2_NOT_NEIGH:
          neighborTypeName = "NOT_NEIGH";
          break;
        case SPLIT2_SYM_NEIGH:
          neighborTypeName = "SYM_NEIGH";
          break;
        case SPLIT2_MPR_NEIGH:
      ...
      if ((lt == SPLIT2_SYM_LINK && nt == SPLIT2_NOT_NEIGH)
          || (nt != SPLIT2_SYM_NEIGH && nt != SPLIT2_MPR_NEIGH
              && nt != SPLIT2_NOT_NEIGH))
        {
...
void
RoutingProtocol::PopulateNeighborSet (const split2::MessageHeader &msg,
                                      const split2::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const split2::MessageHeader &msg,
                                            const split2::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Split2 node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<split2::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == SPLIT2_SYM_NEIGH || neighborType == SPLIT2_MPR_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("Split2 node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}

void
RoutingProtocol::PopulateMprSelectorSet (const split2::MessageHeader &msg,
                                         const split2::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
///
void
SPLIT2::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         SPLIT2::node_id (ra_addr ()),
         SPLIT2::node_id (ch->next_hop ()));
  ...
  SPLIT2_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL)
    {
      link_tuple->lost_time () = now + SPLIT2_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + SPLIT2_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, SPLIT2_WILL_DEFAULT);
...
//         Simulator::Now (),
//         SPLIT2::node_id(ra_addr()),
//         SPLIT2::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == SPLIT2_STATUS_SYM) ? "sym" : "not_sym"));
...
//         SPLIT2::node_id(ra_addr()),
//         SPLIT2::node_id(tuple->neighborMainAddr),
//         SPLIT2::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (SPLIT2_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         SPLIT2::node_id(ra_addr()),
//         SPLIT2::node_id(tuple->main_addr()));
...
//         Simulator::Now (),
//         SPLIT2::node_id(ra_addr()),
//         SPLIT2::node_id(tuple->dest_addr()),
//         SPLIT2::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         SPLIT2::node_id(ra_addr()),
//         SPLIT2::node_id(tuple->main_addr()),
//         SPLIT2::node_id(tuple->iface_addr()));
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (SPLIT2_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber ()
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (SPLIT2_MAX_SEQ_NUM + 1);
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Split2 node " << m_mainAddress
      ...
      if (numOifAddresses == 1)
        ...
        {
          /// \todo Implment IP aliasing and SPLIT2
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and SPLIT2");
        }
