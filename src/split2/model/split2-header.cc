...
#define SPLIT2_MSG_HEADER_SIZE 12
#define SPLIT2_PKT_HEADER_SIZE 4

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("Split2Header");

namespace split2 {


/// Scaling factor used in RFC 3626 (OLSR).
#define SPLIT2_C 0.0625
...
SecondsToEmf (double seconds)
{
  ...
  for (b = 0; (seconds / SPLIT2_C) >= (1 << b); ++b)
    {
    }
  NS_ASSERT ((seconds / SPLIT2_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds / SPLIT2_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16 * (seconds / (SPLIT2_C * (1 << b)) - 1);
...
///
/// \param split2Format number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t split2Format)
{
  int a = (split2Format >> 4);
  int b = (split2Format & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return SPLIT2_C * (1 + a / 16.0) * (1 << b);
}
...
{
  static TypeId tid = TypeId ("ns3::split2::PacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("Split2")
...
{
  static TypeId tid = TypeId ("ns3::split2::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("Split2")
...
{
  uint32_t size = SPLIT2_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = SPLIT2_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - SPLIT2_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - SPLIT2_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - SPLIT2_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - SPLIT2_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
...
{
  return 4 + this->neighborAddresses.size () * (IPV4_ADDRESS_SIZE +1); // ------------- remove +1 when not ETX
}
