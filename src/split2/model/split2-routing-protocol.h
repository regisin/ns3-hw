...
#ifndef SPLIT2_AGENT_IMPL_H
#define SPLIT2_AGENT_IMPL_H

#include "split2-header.h"
#include "ns3/test.h"
#include "split2-state.h"
#include "split2-repositories.h"
...
class Split2MprTestCase;

namespace ns3 {
namespace split2 {

///
/// \defgroup split2 SPLIT2 Routing
/// This section documents the API of the ns-3 SPLIT2 module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup split2
/// An %SPLIT2's routing table entry.
struct RoutingTableEntry
{
  ...
  RoutingTableEntry () : // default values
    ...
    metric(65535), used(false) // ------------------------------ NEW
  {
  }
  
  // ---------------------------------------- START
  void SetEntryUsed (bool val)
  {
    this->used = val;
  }
  // ---------------------------------------- END
...
/// \ingroup split2
///
/// \brief SPLIT2 routing protocol for IPv4
///
class RoutingProtocol : public Ipv4RoutingProtocol
{
public:
  friend class ::Split2MprTestCase;
  ...
  /**
   * \brief Set the SPLIT2 main address to the first address on the indicated interface.
   *
  ...
  /**
   * Return the list of routing table entries discovered by SPLIT2
   */
  std::vector<RoutingTableEntry> GetRoutingTableEntries () const;
  
  
  /**
   * Set all entries 'used' bool to false
   */
  void SetRoutingTableEntriesToUnused (); // --------------------------------- NEW
...
public:
   ...
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "Split2State::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
   ...
   *         to the SPLIT2 routing protocol. Entries from this associated
   *         routing table that use non-split2 outgoing interfaces are added
...
private:
  ...
  /**
   * Receive an SPLIT2 message.
   * \param socket The receiving socket.
   */
  void RecvSplit2 (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-SPLIT2 outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-SPLIT2 interface, false otherwise.
   */
  bool UsesNonSplit2OutgoingInterface (const Ipv4RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the SPLIT2 agent.
   *
  ...
  split2::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief SPLIT2's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param split2Message The %SPLIT2 message which must be forwarded.
   ...
   */
  void ForwardDefault (split2::MessageHeader split2Message,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);

  /**
   * \brief Enques an %SPLIT2 message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %SPLIT2 messages in
   * a same %SPLIT2 packet.
   *
   * \param message the %SPLIT2 message which must be sent.
   * \param delay maximum delay the %SPLIT2 message is going to be buffered.
   */
  void QueueMessage (const split2::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %SPLIT2 packets as needed in order to send all buffered
   * %SPLIT2 messages.
   *
   * Maximum number of messages which can be contained in an %SPLIT2 packet is
   * dictated by SPLIT2_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %SPLIT2 HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %SPLIT2 TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %SPLIT2 MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %SPLIT2 HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %SPLIT2 message which contains the HELLO message.
   ...
   */
  void ProcessHello (const split2::MessageHeader &msg,
   ...
   *
   * \param msg The %SPLIT2 message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const split2::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %SPLIT2 message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const split2::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %SPLIT2 message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const split2::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const split2::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const split2::MessageHeader &msg,
                            const split2::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const split2::MessageHeader &msg,
                                  const split2::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const split2::MessageHeader &msg,
                               const split2::MessageHeader::Hello &hello);
  ...
  // (reason: for SPLIT2 Link Sensing we need to know on which interface
  ...
  //------------------------------------ split2 stuff
  // Get a pointer to a specific wifinetdevice, given a local address
