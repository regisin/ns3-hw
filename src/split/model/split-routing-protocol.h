...
#ifndef SPLIT_AGENT_IMPL_H
#define SPLIT_AGENT_IMPL_H
...
class SplitMprTestCase;

namespace ns3 {
namespace split {

///
/// \defgroup split SPLIT Routing
/// This section documents the API of the ns-3 SPLIT module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup split
/// An %SPLIT's routing table entry.
...
/// \ingroup split
///
/// \brief SPLIT routing protocol for IPv4
///
class RoutingProtocol : public Ipv4RoutingProtocol
{
public:
  friend class ::SplitMprTestCase;
  ...
  /**
   * \brief Set the SPLIT main address to the first address on the indicated interface.
   *
  ...
  /**
   * Return the list of routing table entries discovered by SPLIT
   */
...
public:
   ...
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "SplitState::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
   ...
   *         to the SPLIT routing protocol. Entries from this associated
   *         routing table that use non-split outgoing interfaces are added
...
private:
  ...
  /**
   * Receive an SPLIT message.
   * \param socket The receiving socket.
   */
  void RecvSplit (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-SPLIT outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-SPLIT interface, false otherwise.
   */
  bool UsesNonSplitOutgoingInterface (const Ipv4RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the SPLIT agent.
   *
  ...
  split::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief SPLIT's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param splitMessage The %SPLIT message which must be forwarded.
   ...
   */
  void ForwardDefault (split::MessageHeader splitMessage,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);

  /**
   * \brief Enques an %SPLIT message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %SPLIT messages in
   * a same %SPLIT packet.
   *
   * \param message the %SPLIT message which must be sent.
   * \param delay maximum delay the %SPLIT message is going to be buffered.
   */
  void QueueMessage (const split::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %SPLIT packets as needed in order to send all buffered
   * %SPLIT messages.
   *
   * Maximum number of messages which can be contained in an %SPLIT packet is
   * dictated by SPLIT_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %SPLIT HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %SPLIT TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %SPLIT MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %SPLIT HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %SPLIT message which contains the HELLO message.
   ...
   */
  void ProcessHello (const split::MessageHeader &msg,
   ...
   *
   * \param msg The %SPLIT message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const split::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %SPLIT message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const split::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %SPLIT message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const split::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const split::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const split::MessageHeader &msg,
                            const split::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const split::MessageHeader &msg,
                                  const split::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const split::MessageHeader &msg,
                               const split::MessageHeader::Hello &hello);
  ...
  // (reason: for SPLIT Link Sensing we need to know on which interface
