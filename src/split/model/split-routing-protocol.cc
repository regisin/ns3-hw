...
///
/// \brief Implementation of SPLIT agent and related classes.
///
/// This is the main file of this software because %SPLIT's behaviour is
/// implemented here.
///
...
#include "split-routing-protocol.h"
...
#include "ns3/split-routing-protocol.h"
...
#include "src/wifi/model/wifi-phy.h"
#include "src/energy/model/energy-source.h"
...
///
/// We only use this value in order to define SPLIT_NEIGHB_HOLD_TIME.
///
#define SPLIT_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define SPLIT_NEIGHB_HOLD_TIME   Time (3 * SPLIT_REFRESH_INTERVAL)
/// Top holding time.
#define SPLIT_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define SPLIT_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define SPLIT_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define SPLIT_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define SPLIT_UNSPEC_LINK        0
/// Asymmetric link type.
#define SPLIT_ASYM_LINK          1
/// Symmetric link type.
#define SPLIT_SYM_LINK           2
/// Lost link type.
#define SPLIT_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define SPLIT_NOT_NEIGH          0
/// Symmetric neighbor type.
#define SPLIT_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define SPLIT_MPR_NEIGH          2


/********** Willingness **********/

/// Willingness for forwarding packets from other nodes: never.
#define SPLIT_WILL_NEVER         0
/// Willingness for forwarding packets from other nodes: low.
#define SPLIT_WILL_LOW           1
/// Willingness for forwarding packets from other nodes: medium.
#define SPLIT_WILL_DEFAULT       3
/// Willingness for forwarding packets from other nodes: high.
#define SPLIT_WILL_HIGH          6
/// Willingness for forwarding packets from other nodes: always.
#define SPLIT_WILL_ALWAYS        7


/********** Miscellaneous constants **********/

/// Maximum allowed jitter.
#define SPLIT_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define SPLIT_MAX_SEQ_NUM        65535
/// Random number between [0-SPLIT_MAXJITTER] used to jitter SPLIT packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, SPLIT_MAXJITTER)))


#define SPLIT_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define SPLIT_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define SPLIT_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define SPLIT_MAX_ADDRS          64


// ------------------------------------- START
///Constants for  ETX Metrics
#define SPLIT_MAXIMUM_METRIC 65535
#define SPLIT_DEFAULT_METRIC 4096
#define SPLIT_UNDEFINED_LQ 0
#define SPLIT_ETX_SEQNO_RESTART_DETECTION 256

#define SPLIT_ETX_UPDATE_TIME   Time (7 * SPLIT_REFRESH_INTERVAL)
// ------------------------------------- END


namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("SplitRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::split::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   ...
                   MakeEnumChecker (SPLIT_WILL_NEVER, "never",
                                    SPLIT_WILL_LOW, "low",
                                    SPLIT_WILL_DEFAULT, "default",
                                    SPLIT_WILL_HIGH, "high",
                                    SPLIT_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive SPLIT packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::split::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send SPLIT packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace),
                     "ns3::split::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The SPLIT routing table has changed.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_routingTableChanged),
                     "ns3::split::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created split::RoutingProtocol");
  ...
  m_packetSequenceNumber = SPLIT_MAX_SEQ_NUM;
  m_messageSequenceNumber = SPLIT_MAX_SEQ_NUM;
  m_ansn = SPLIT_MAX_SEQ_NUM;
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream) const
{
  ...
  *os << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
      ...
      << ", SPLIT Routing table" << std::endl;

  *os << "Destination\t\tNextHop\t\tInterface\tDistance\tMetric\n";
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting SPLIT on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunSplit = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), SPLIT_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvSplit,  this));
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() SPLIT socket");
        }
  ...
  if (canRunSplit)
    {
      ...
      NS_LOG_DEBUG ("SPLIT on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %SPLIT packet following \RFC{3626} specification.
void
RoutingProtocol::RecvSplit (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("SPLIT node " << m_mainAddress << " received a SPLIT packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == SPLIT_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  split::PacketHeader splitPacketHeader;
  packet->RemoveHeader (splitPacketHeader);
  NS_ASSERT (splitPacketHeader.GetPacketLength () >= splitPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = splitPacketHeader.GetPacketLength () - splitPacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Split Msg received with type "
  ...
  m_rxPacketTrace (splitPacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case split::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SPLIT node " << m_mainAddress
            ...
            case split::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SPLIT node " << m_mainAddress
            ...
            default:
              NS_LOG_DEBUG ("SPLIT message type " <<
        ...
        {
          NS_LOG_DEBUG ("SPLIT message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != split::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all SPLIT messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == SPLIT_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == SPLIT_WILL_ALWAYS)
        {
...
RoutingProtocol::RoutingTableComputation ()
{
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != SPLIT_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
  ...
  for (uint32_t h = 2;; h++)
    {
      ...
      std::vector<RoutingTableEntry> rtEntries = GetRoutingTableEntries();

      static RoutingTableEntry current_entry;
      for (std::vector<RoutingTableEntry>::iterator rte = rtEntries.begin (); rte != rtEntries.end (); rte++)
      {
        Ipv4Address destAddr = rte->destAddr;
        Ipv4Address nextAddr = rte->nextAddr;
        uint32_t interface   = rte->interface;
        uint32_t distance    = rte->distance;
        uint32_t metric      = rte->metric;

        if (!rte->used)
        {
          RemoveEntry(destAddr);
          AddEntry (destAddr, nextAddr, interface, distance, metric, true);
          Lookup (destAddr, current_entry);
          isUnused = true;
          //NS_LOG_UNCOND("in 1st loop AFTER destAddr [" << current_entry.destAddr << "] nextAddr [" << current_entry.nextAddr << "] used [" << current_entry.used << "]");
          break;
        }
      }
      ...
      if (isUnused)
      {
        ...
        for (TopologySet::const_iterator it = topology.begin (); it != topology.end (); it++)
        {
          ...
          if (topology_tuple.lastAddr == current_entry.destAddr)
          {
            ...
            {
              if (topology_tuple.destAddr != m_mainAddress)
              {
                AddEntry (topology_tuple.destAddr,
                          current_entry.nextAddr,
                          current_entry.interface,
                          new_distance,
                          new_metric,
                          false);
              }
            }
            else if (new_metric < destAddrEntry.metric)
            {
              if (topology_tuple.destAddr != m_mainAddress)
              {
                RemoveEntry(topology_tuple.destAddr);
                AddEntry(topology_tuple.destAddr, current_entry.nextAddr, current_entry.interface, new_distance, new_metric, true);
              }
            }
          }
        }
  ...
  SetRoutingTableEntriesToUnused(); // ------------------------------------------- NEW
...
void
RoutingProtocol::ProcessHello (const split::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const split::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Link Set for SPLIT Node " << m_mainAddress);
    for (LinkSet::const_iterator link = links.begin (); link != links.end (); link++)
      {
        NS_LOG_DEBUG (*link);
      }
    NS_LOG_DEBUG ("** END dump Link Set for SPLIT Node " << m_mainAddress);

    const NeighborSet &neighbors = m_state.GetNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Neighbor Set for SPLIT Node " << m_mainAddress);
    for (NeighborSet::const_iterator neighbor = neighbors.begin (); neighbor != neighbors.end (); neighbor++)
      {
        NS_LOG_DEBUG (*neighbor);
      }
    NS_LOG_DEBUG ("** END dump Neighbor Set for SPLIT Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for SPLIT Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for SPLIT Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
void
RoutingProtocol::ProcessTc (const split::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const split::MessageHeader::Tc &tc = msg.GetTc ();
...
//       i != tc.neighborAddresses.end (); i++)
  for(uint8_t i = 0; i < tc.neighborAddresses.size(); i++) // ------------------------ NEW
    {
//      const Ipv4Address &addr = *i;
      const Ipv4Address &addr = tc.neighborAddresses.at(i);  // ------------------------ NEW
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for SPLIT Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for SPLIT Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}

void
RoutingProtocol::ProcessMid (const split::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const split::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const split::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const split::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<split::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (split::MessageHeader splitMessage,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << splitMessage.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (splitMessage.GetTimeToLive () > 1)
    {
      const MprSelectorTuple *mprselTuple =
        m_state.FindMprSelectorTuple (GetMainAddress (senderAddress));
      if (mprselTuple != NULL)
        {
          splitMessage.SetTimeToLive (splitMessage.GetTimeToLive () - 1);
          splitMessage.SetHopCount (splitMessage.GetHopCount () + 1);
          // We have to introduce a random delay to avoid
          // synchronization with neighbors.
          QueueMessage (splitMessage, JITTER);
          retransmitted = true;
        }
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + SPLIT_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = splitMessage.GetOriginatorAddress ();
      newDup.sequenceNumber = splitMessage.GetMessageSequenceNumber ();
      newDup.expirationTime = now + SPLIT_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (SPLIT_DUP_HOLD_TIME,
...
void
RoutingProtocol::QueueMessage (const split::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("SPLIT node " << m_mainAddress << " sending a SPLIT packet");

  // Add a header
  split::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      i->first->SendTo (packet, 0, InetSocketAddress (bcast, SPLIT_PORT_NUMBER));
    }
}
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("Split node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<split::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  split::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (SPLIT_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  split::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<split::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      split::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("SPLIT HELLO message size: " << int (msg.GetSerializedSize ())
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (SPLIT_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  split::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetVTime (SPLIT_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (SPLIT_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  split::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<split::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      split::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-split interface
          if (UsesNonSplitOutgoingInterface (route))
            {
  ...
  // add the routes using non-split outgoing interfaces to the list
...
bool
RoutingProtocol::UsesNonSplitOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-SPLIT interface if a match is found
  // before reaching the end of the list of excluded interfaces
  return ci != m_interfaceExclusions.end ();
}

void
RoutingProtocol::LinkSensing (const split::MessageHeader &msg,
                              const split::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  ...
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Split node " << m_mainAddress
  ...
  if (link_tuple == NULL)
    {
      ...
      newLinkTuple.RLQ = 1;
      newLinkTuple.DLQ = SPLIT_UNDEFINED_LQ;
      newLinkTuple.ETX = SPLIT_DEFAULT_METRIC;
      // ----------------------------------------------------------------------- END
  ...
  else
    {
      ...
      if (diff > SPLIT_ETX_SEQNO_RESTART_DETECTION)
      {
        diff = 1;
      }
  ...
  for (std::vector<split::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (nt)
        {
        case SPLIT_NOT_NEIGH:
          neighborTypeName = "NOT_NEIGH";
          break;
        case SPLIT_SYM_NEIGH:
          neighborTypeName = "SYM_NEIGH";
          break;
        case SPLIT_MPR_NEIGH:
      ...
      if ((lt == SPLIT_SYM_LINK && nt == SPLIT_NOT_NEIGH)
          || (nt != SPLIT_SYM_NEIGH && nt != SPLIT_MPR_NEIGH
              && nt != SPLIT_NOT_NEIGH))
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == SPLIT_LOST_LINK)
              ...
              else if (lt == SPLIT_SYM_LINK || lt == SPLIT_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + SPLIT_NEIGHB_HOLD_TIME;
  ...
  if (link_tuple->DLQ == SPLIT_UNDEFINED_LQ)
  {
    link_tuple->ETX = SPLIT_DEFAULT_METRIC;
  }
  else
  {
      // split stuff
  ...
  if (created)
    {
      ...
      m_events.Track (Simulator::Schedule (DELAY (std::min (link_tuple->time, link_tuple->symTime)),
                                           ...
                                           link_tuple->neighborIfaceAddr));
      // Schedule ETX timer
      m_events.Track (Simulator::Schedule (SPLIT_ETX_UPDATE_TIME, // -------------------------------- NEW
...
void
RoutingProtocol::PopulateNeighborSet (const split::MessageHeader &msg,
                                      const split::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const split::MessageHeader &msg,
                                            const split::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Split node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<split::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == SPLIT_SYM_NEIGH || neighborType == SPLIT_MPR_NEIGH)
                {
              ...
              else if (neighborType == SPLIT_NOT_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("Split node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}

void
RoutingProtocol::PopulateMprSelectorSet (const split::MessageHeader &msg,
                                         const split::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
SPLIT::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         SPLIT::node_id (ra_addr ()),
         SPLIT::node_id (ch->next_hop ()));
  ...
  SPLIT_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL)
    {
      link_tuple->lost_time () = now + SPLIT_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + SPLIT_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, SPLIT_WILL_DEFAULT);
...
//         Simulator::Now (),
//         SPLIT::node_id(ra_addr()),
//         SPLIT::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == SPLIT_STATUS_SYM) ? "sym" : "not_sym"));
...
//         SPLIT::node_id(ra_addr()),
//         SPLIT::node_id(tuple->neighborMainAddr),
//         SPLIT::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (SPLIT_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         SPLIT::node_id(ra_addr()),
//         SPLIT::node_id(tuple->dest_addr()),
//         SPLIT::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         SPLIT::node_id(ra_addr()),
//         SPLIT::node_id(tuple->main_addr()),
//         SPLIT::node_id(tuple->iface_addr()));
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (SPLIT_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber ()
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (SPLIT_MAX_SEQ_NUM + 1);
...
RoutingProtocol::EtxTimerExpire (Ipv4Address neighborIfaceAddr)
{
  ...
  m_events.Track (Simulator::Schedule (SPLIT_ETX_UPDATE_TIME,
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Split node " << m_mainAddress
      ...
      if (numOifAddresses == 1)
        ...
        {
          /// \todo Implment IP aliasing and SPLIT
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and SPLIT");
        }
...
RoutingProtocol::AddEntry (Ipv4Address const &dest,
                           ...
                           bool used)
{
  NS_LOG_FUNCTION (this << dest << next << interface << distance << metric << used << m_mainAddress); // -- modified

  NS_ASSERT (distance > 0 && metric >= 0); // ----------------------------------- Remove metric condition for OLSR
...
RoutingProtocol::AddEntry (Ipv4Address const &dest,
                           ...
                           bool used) // --------------------------------------- NEW
{
  NS_LOG_FUNCTION (this << dest << next << interfaceAddress << distance << metric << used << m_mainAddress); // -- modified
      ...
      for (uint32_t j = 0; j < m_ipv4->GetNAddresses (i); j++)
        {
          if (m_ipv4->GetAddress (i,j).GetLocal () == interfaceAddress)
            {
              AddEntry (dest, next, i, distance, metric, used); // ------------- modified
              return;
            }
        }
    }
  NS_ASSERT (false); // should not be reached
  AddEntry (dest, next, 0, distance, metric, used); // ------------------------- modified
}
...
// ----------------------------------------------------------------------------- START
void
RoutingProtocol::SetRoutingTableEntriesToUnused ()
{
    std::vector<RoutingTableEntry> rtEntries = GetRoutingTableEntries();

    for (std::vector<RoutingTableEntry>::iterator rte = rtEntries.begin (); rte != rtEntries.end (); rte++)
    {
        RoutingTableEntry *entry = &(*rte);
        entry->SetEntryUsed(false);
    }
}
// ----------------------------------------------------------------------------- END
...
Ptr<NetDevice>
RoutingProtocol::GetNetDevice(const Ipv4Address &addr)
{
    Ptr<NetDevice> device;
    for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
    {
        if (addr == i->second.GetLocal()) {
            device = i->first->GetBoundNetDevice();
        }
    }
    return device;
}
