...
#define SPLIT_MSG_HEADER_SIZE 12
#define SPLIT_PKT_HEADER_SIZE 4

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("SplitHeader");

namespace split {


/// Scaling factor used in RFC 3626 (OLSR).
#define SPLIT_C 0.0625
...
SecondsToEmf (double seconds)
{
  ...
  for (b = 0; (seconds / SPLIT_C) >= (1 << b); ++b)
    {
    }
  NS_ASSERT ((seconds / SPLIT_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds / SPLIT_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16 * (seconds / (SPLIT_C * (1 << b)) - 1);
...
///
/// \param splitFormat number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t splitFormat)
{
  int a = (splitFormat >> 4);
  int b = (splitFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return SPLIT_C * (1 + a / 16.0) * (1 << b);
}
...
double
EmfToEtxVal (uint8_t splitFormat)
{
    
    int b = (splitFormat >> 3);
    int a = (splitFormat & 0x7);
...
{
  static TypeId tid = TypeId ("ns3::split::PacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("Split")
...
{
  static TypeId tid = TypeId ("ns3::split::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("Split")
...
{
  uint32_t size = SPLIT_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = SPLIT_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - SPLIT_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - SPLIT_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - SPLIT_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - SPLIT_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
