...
{
    if (m_energyDevice->GetRemainingEnergy () > 0.0)
    {
...
//********************************************************************
void
SimpleWirelessNetDevice::SetCurrentDown()
{
    m_energyDevice->SetCurrentA (0.1);
}
void
SimpleWirelessNetDevice::SetCurrentUp()
{
    double C = m_bps.GetBitRate ();
    double Prx = pow(2,C / (m_channel->GetChannelWidth()*1000000))-1.0;
    double lamb = 300000000.0 / m_channel->GetChannelFreq(); // speed of light assumed 3^8 m/s
    double Ptx = Prx / (lamb / (4*3.1415926535897*pow(m_channel->GetMaxRange(),2)));

    double V = m_energyDevice->GetSupplyVoltage();

    double I = Ptx / V;
    m_energyDevice->SetCurrentA (I);
//    m_energyDevice->SetCurrentA (0.5);
}
...
SimpleWirelessNetDevice::TransmitComplete (void)
{
  ...
  NS_ASSERT_MSG (item != 0, "SimpleWirelessNetDevice::TransmitAbort(): IsEmpty false but no Packet on queue?");
...
SimpleWirelessNetDevice::EnqueuePacket (Ptr<Packet> packet, Mac48Address from, Mac48Address to, uint16_t protocolNumber, uint32_t destId)
{
    if (m_energyDevice->GetRemainingEnergy () > 0.0)
    {
        ...
        else
        {
           ...
           Simulator::Schedule (txTime, &SimpleWirelessNetDevice::SetCurrentDown, this);
...
void
SimpleWirelessNetDevice::SetEnergyDevice (Ptr<SimpleWirelessEnergyModel> e)
{
  NS_LOG_FUNCTION (this << e);
  m_energyDevice = e;
}

Ptr<SimpleWirelessEnergyModel>
SimpleWirelessNetDevice::GetEnergyDevice (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_energyDevice;
}
