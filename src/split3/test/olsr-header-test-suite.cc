...
#include "ns3/test.h"
#include "ns3/split3-header.h"
#include "ns3/packet.h"

using namespace ns3;

class Split3EmfTestCase : public TestCase
{
public:
  Split3EmfTestCase ();
...
Split3EmfTestCase::Split3EmfTestCase ()
  : TestCase ("Check Emf split3 time conversion")
{
}
void
Split3EmfTestCase::DoRun (void)
{
  for (int time = 1; time <= 30; time++)
    {
      uint8_t emf = split3::SecondsToEmf (time);
      double seconds = split3::EmfToSeconds (emf);
...
class Split3MidTestCase : public TestCase
{
public:
  Split3MidTestCase ();
  virtual void DoRun (void);
...
Split3MidTestCase::Split3MidTestCase ()
  : TestCase ("Check Mid split3 messages")
{
}
void
Split3MidTestCase::DoRun (void)
{
  Packet packet;

  {
    split3::PacketHeader hdr;
    split3::MessageHeader msg1;
    split3::MessageHeader::Mid &mid1 = msg1.GetMid ();
    split3::MessageHeader msg2;
    split3::MessageHeader::Mid &mid2 = msg2.GetMid ();
    ...
    msg2.SetMessageType (split3::MessageHeader::MID_MESSAGE);
    msg2.SetMessageSequenceNumber (7);

    // Build an SPLIT3 packet header
...

  {
    ...
    uint32_t sizeLeft = hdr.GetPacketLength () - hdr.GetSerializedSize ();
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg1.GetMessageType (), split3::MessageHeader::MID_MESSAGE, "204");
    ...
    }
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg2.GetMessageType (), split3::MessageHeader::MID_MESSAGE, "212");
...
class Split3HelloTestCase : public TestCase
{
public:
  Split3HelloTestCase ();
  virtual void DoRun (void);
...
Split3HelloTestCase::Split3HelloTestCase ()
  : TestCase ("Check Hello split3 messages")
{
}
void
Split3HelloTestCase::DoRun (void)
{
  Packet packet;
  split3::MessageHeader msgIn;
  split3::MessageHeader::Hello &helloIn = msgIn.GetHello ();

  helloIn.SetHTime (Seconds (7));
  helloIn.willingness = 66;

  {
    split3::MessageHeader::Hello::LinkMessage lm1;
    lm1.linkCode = 2;
    ...
    split3::MessageHeader::Hello::LinkMessage lm2;
    lm2.linkCode = 3;
  ...
  split3::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  split3::MessageHeader::Hello &helloOut = msgOut.GetHello ();
...
class Split3TcTestCase : public TestCase
{
public:
  Split3TcTestCase ();
  virtual void DoRun (void);
...
Split3TcTestCase::Split3TcTestCase ()
  : TestCase ("Check Tc split3 messages")
{
}
void
Split3TcTestCase::DoRun (void)
{
  Packet packet;
  split3::MessageHeader msgIn;
  split3::MessageHeader::Tc &tcIn = msgIn.GetTc ();
  ...
  split3::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  split3::MessageHeader::Tc &tcOut = msgOut.GetTc ();
...
class Split3HnaTestCase : public TestCase
{
public:
  Split3HnaTestCase ();
  virtual void DoRun (void);
...
Split3HnaTestCase::Split3HnaTestCase ()
  : TestCase ("Check Hna split3 messages")
{
}

void
Split3HnaTestCase::DoRun (void)
{
  Packet packet;
  split3::MessageHeader msgIn;
  split3::MessageHeader::Hna &hnaIn = msgIn.GetHna ();

  hnaIn.associations.push_back ((split3::MessageHeader::Hna::Association)
  ...
  split3::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  split3::MessageHeader::Hna &hnaOut = msgOut.GetHna ();
...
static class Split3TestSuite : public TestSuite
{
public:
  Split3TestSuite ();
} g_split3TestSuite;

Split3TestSuite::Split3TestSuite ()
  : TestSuite ("routing-split3-header", UNIT)
{
  AddTestCase (new Split3HnaTestCase (), TestCase::QUICK);
  AddTestCase (new Split3TcTestCase (), TestCase::QUICK);
  AddTestCase (new Split3HelloTestCase (), TestCase::QUICK);
  AddTestCase (new Split3MidTestCase (), TestCase::QUICK);
  AddTestCase (new Split3EmfTestCase (), TestCase::QUICK);
}
