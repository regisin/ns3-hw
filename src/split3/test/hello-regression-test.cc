...
HelloRegressionTest::HelloRegressionTest () :
  TestCase ("Test SPLIT3 Hello messages generation"),
...
HelloRegressionTest::CreateNodes ()
{
  ...
  NodeContainer c;
  c.Create (2);
  // install TCP/IP & SPLIT3
  Split3Helper split3;
  InternetStackHelper internet;
  internet.SetRoutingHelper (split3);
  internet.Install (c);
  // Assign SPLIT3 RVs to specific streams
  int64_t streamsUsed = split3.AssignStreams (c, 0);
...
HelloRegressionTest::ReceivePktProbeA (Ptr<Socket> socket)
{
  ...
  const split3::MessageHeader::Hello &hello = msgHdr.GetHello ();
  ...
  std::vector<split3::MessageHeader::Hello::LinkMessage>::const_iterator iter;
