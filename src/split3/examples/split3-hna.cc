...
//
// One way is to use split3::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "split3-hna --assocMethod1=1"
//
// The other way is to use split3::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "split3-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("Split3Hna");
...
int main (int argc, char *argv[])
{
  ...
  NodeContainer split3Nodes;
  split3Nodes.Create (2);
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, split3Nodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), split3Nodes.Get (1)));
  ...
  mobility.Install (split3Nodes);

  Split3Helper split3;

  // Specify Node B's csma device as a non-SPLIT3 device.
  split3.ExcludeInterface (split3Nodes.Get (1), 2);
  ...
  InternetStackHelper internet_split3;
  internet_split3.SetRoutingHelper (list); // has effect on the next Install ()
  internet_split3.Install (split3Nodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (split3Nodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain split3::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = split3Nodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<split3::RoutingProtocol> split3rp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<split3::RoutingProtocol> (temp))
        {
          split3rp_Gw = DynamicCast<split3::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-SPLIT3 interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      split3rp_Gw->SetRoutingTableAssociation (hnaEntries);
    }

  if (assocMethod2)
    {
      // Specify the required associations directly.
      split3rp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("split3-hna", devices);
  csma.EnablePcap ("split3-hna", csmaDevices, false);
