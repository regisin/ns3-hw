...
///
/// \brief Implementation of SPLIT3 agent and related classes.
///
/// This is the main file of this software because %SPLIT3's behaviour is
/// implemented here.
///
...
#include "split3-routing-protocol.h"
...
///
/// We only use this value in order to define SPLIT3_NEIGHB_HOLD_TIME.
///
#define SPLIT3_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define SPLIT3_NEIGHB_HOLD_TIME   Time (3 * SPLIT3_REFRESH_INTERVAL)
/// Top holding time.
#define SPLIT3_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define SPLIT3_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define SPLIT3_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define SPLIT3_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define SPLIT3_UNSPEC_LINK        0
/// Asymmetric link type.
#define SPLIT3_ASYM_LINK          1
/// Symmetric link type.
#define SPLIT3_SYM_LINK           2
/// Lost link type.
#define SPLIT3_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define SPLIT3_NOT_NEIGH          0
/// Symmetric neighbor type.
#define SPLIT3_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define SPLIT3_MPR_NEIGH          2
...
#define SPLIT3_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define SPLIT3_MAX_SEQ_NUM        65535
/// Random number between [0-SPLIT3_MAXJITTER] used to jitter SPLIT3 packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, SPLIT3_MAXJITTER)))


#define SPLIT3_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define SPLIT3_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define SPLIT3_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define SPLIT3_MAX_ADDRS          64


namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("Split3RoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::split3::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (SPLIT3_WILL_DEFAULT),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (SPLIT3_WILL_NEVER, "never",
                                    SPLIT3_WILL_LOW, "low",
                                    SPLIT3_WILL_DEFAULT, "default",
                                    SPLIT3_WILL_HIGH, "high",
                                    SPLIT3_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive SPLIT3 packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::split3::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send SPLIT3 packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace),
                     "ns3::split3::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The SPLIT3 routing table has changed.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_routingTableChanged),
                     "ns3::split3::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created split3::RoutingProtocol");
  ...
  m_packetSequenceNumber = SPLIT3_MAX_SEQ_NUM;
  m_messageSequenceNumber = SPLIT3_MAX_SEQ_NUM;
  m_ansn = SPLIT3_MAX_SEQ_NUM;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting SPLIT3 on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunSplit3 = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), SPLIT3_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvSplit3,  this));
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() SPLIT3 socket");
        }
  ...
  if (canRunSplit3)
    {
      ...
      NS_LOG_DEBUG ("SPLIT3 on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %SPLIT3 packet following \RFC{3626} specification.
void
RoutingProtocol::RecvSplit3 (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("SPLIT3 node " << m_mainAddress << " received a SPLIT3 packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == SPLIT3_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  split3::PacketHeader split3PacketHeader;
  packet->RemoveHeader (split3PacketHeader);
  NS_ASSERT (split3PacketHeader.GetPacketLength () >= split3PacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = split3PacketHeader.GetPacketLength () - split3PacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Split3 Msg received with type "
  ...
  m_rxPacketTrace (split3PacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case split3::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SPLIT3 node " << m_mainAddress
            ...
            case split3::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SPLIT3 node " << m_mainAddress
            ...
            default:
              NS_LOG_DEBUG ("SPLIT3 message type " <<
        ...
        {
          NS_LOG_DEBUG ("SPLIT3 message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != split3::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all SPLIT3 messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == SPLIT3_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == SPLIT3_WILL_ALWAYS)
        {
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != SPLIT3_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
...
void
RoutingProtocol::ProcessHello (const split3::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const split3::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Link Set for SPLIT3 Node " << m_mainAddress);
    for (LinkSet::const_iterator link = links.begin (); link != links.end (); link++)
      {
        NS_LOG_DEBUG (*link);
      }
    NS_LOG_DEBUG ("** END dump Link Set for SPLIT3 Node " << m_mainAddress);

    const NeighborSet &neighbors = m_state.GetNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Neighbor Set for SPLIT3 Node " << m_mainAddress);
    for (NeighborSet::const_iterator neighbor = neighbors.begin (); neighbor != neighbors.end (); neighbor++)
      {
        NS_LOG_DEBUG (*neighbor);
      }
    NS_LOG_DEBUG ("** END dump Neighbor Set for SPLIT3 Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for SPLIT3 Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for SPLIT3 Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
void
RoutingProtocol::ProcessTc (const split3::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const split3::MessageHeader::Tc &tc = msg.GetTc ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for SPLIT3 Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for SPLIT3 Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}

void
RoutingProtocol::ProcessMid (const split3::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const split3::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const split3::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const split3::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<split3::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (split3::MessageHeader split3Message,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << split3Message.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (split3Message.GetTimeToLive () > 1)
    {
      const MprSelectorTuple *mprselTuple =
        m_state.FindMprSelectorTuple (GetMainAddress (senderAddress));
      if (mprselTuple != NULL)
        {
          split3Message.SetTimeToLive (split3Message.GetTimeToLive () - 1);
          split3Message.SetHopCount (split3Message.GetHopCount () + 1);
          // We have to introduce a random delay to avoid
          // synchronization with neighbors.
          QueueMessage (split3Message, JITTER);
          retransmitted = true;
        }
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + SPLIT3_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = split3Message.GetOriginatorAddress ();
      newDup.sequenceNumber = split3Message.GetMessageSequenceNumber ();
      newDup.expirationTime = now + SPLIT3_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (SPLIT3_DUP_HOLD_TIME,
...
void
RoutingProtocol::QueueMessage (const split3::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("SPLIT3 node " << m_mainAddress << " sending a SPLIT3 packet");

  // Add a header
  split3::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      i->first->SendTo (packet, 0, InetSocketAddress (bcast, SPLIT3_PORT_NUMBER));
    }
}
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("Split3 node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<split3::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
       ...
       message++)
    {
      ...
      if (++numMessages == SPLIT3_MAX_MSGS)
        {
...
RoutingProtocol::SendHello ()
{
  ...
  split3::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (SPLIT3_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  split3::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<split3::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      split3::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("SPLIT3 HELLO message size: " << int (msg.GetSerializedSize ())
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (SPLIT3_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  split3::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetVTime (SPLIT3_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (SPLIT3_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  split3::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<split3::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      split3::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-split3 interface
          if (UsesNonSplit3OutgoingInterface (route))
            {
  ...
  // add the routes using non-split3 outgoing interfaces to the list
...
bool
RoutingProtocol::UsesNonSplit3OutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-SPLIT3 interface if a match is found
  // before reaching the end of the list of excluded interfaces
  return ci != m_interfaceExclusions.end ();
}

void
RoutingProtocol::LinkSensing (const split3::MessageHeader &msg,
                              const split3::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Split3 node " << m_mainAddress
  ...
  for (std::vector<split3::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (lt)
        {
        case SPLIT3_UNSPEC_LINK:
          linkTypeName = "UNSPEC_LINK";
          break;
        case SPLIT3_ASYM_LINK:
          linkTypeName = "ASYM_LINK";
          break;
        case SPLIT3_SYM_LINK:
          linkTypeName = "SYM_LINK";
          break;
        case SPLIT3_LOST_LINK:
      ...
      switch (nt)
        {
        case SPLIT3_NOT_NEIGH:
          neighborTypeName = "NOT_NEIGH";
          break;
        case SPLIT3_SYM_NEIGH:
          neighborTypeName = "SYM_NEIGH";
          break;
        case SPLIT3_MPR_NEIGH:
      ...
      if ((lt == SPLIT3_SYM_LINK && nt == SPLIT3_NOT_NEIGH)
          || (nt != SPLIT3_SYM_NEIGH && nt != SPLIT3_MPR_NEIGH
              && nt != SPLIT3_NOT_NEIGH))
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == SPLIT3_LOST_LINK)
              ...
              else if (lt == SPLIT3_SYM_LINK || lt == SPLIT3_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + SPLIT3_NEIGHB_HOLD_TIME;
                  updated = true;
                }
              else
                {
...
void
RoutingProtocol::PopulateNeighborSet (const split3::MessageHeader &msg,
                                      const split3::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const split3::MessageHeader &msg,
                                            const split3::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Split3 node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<split3::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == SPLIT3_SYM_NEIGH || neighborType == SPLIT3_MPR_NEIGH)
                {
              ...
              else if (neighborType == SPLIT3_NOT_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("Split3 node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}

void
RoutingProtocol::PopulateMprSelectorSet (const split3::MessageHeader &msg,
                                         const split3::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
///
void
SPLIT3::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         SPLIT3::node_id (ra_addr ()),
         SPLIT3::node_id (ch->next_hop ()));
  ...
  SPLIT3_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL)
    {
      link_tuple->lost_time () = now + SPLIT3_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + SPLIT3_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, SPLIT3_WILL_DEFAULT);
...
//         Simulator::Now (),
//         SPLIT3::node_id(ra_addr()),
//         SPLIT3::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == SPLIT3_STATUS_SYM) ? "sym" : "not_sym"));
...
//         SPLIT3::node_id(ra_addr()),
//         SPLIT3::node_id(tuple->neighborMainAddr),
//         SPLIT3::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (SPLIT3_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         SPLIT3::node_id(ra_addr()),
//         SPLIT3::node_id(tuple->main_addr()));
...
//         Simulator::Now (),
//         SPLIT3::node_id(ra_addr()),
//         SPLIT3::node_id(tuple->dest_addr()),
//         SPLIT3::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         SPLIT3::node_id(ra_addr()),
//         SPLIT3::node_id(tuple->main_addr()),
//         SPLIT3::node_id(tuple->iface_addr()));
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (SPLIT3_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber ()
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (SPLIT3_MAX_SEQ_NUM + 1);
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Split3 node " << m_mainAddress
      ...
      if (numOifAddresses == 1)
        ...
        {
          /// \todo Implment IP aliasing and SPLIT3
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and SPLIT3");
        }
