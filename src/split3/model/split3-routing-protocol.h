...
#ifndef SPLIT3_AGENT_IMPL_H
#define SPLIT3_AGENT_IMPL_H

#include "split3-header.h"
#include "ns3/test.h"
#include "split3-state.h"
#include "split3-repositories.h"
...
class Split3MprTestCase;

namespace ns3 {
namespace split3 {

///
/// \defgroup split3 SPLIT3 Routing
/// This section documents the API of the ns-3 SPLIT3 module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup split3
/// An %SPLIT3's routing table entry.
...
/// \ingroup split3
///
/// \brief SPLIT3 routing protocol for IPv4
///
class RoutingProtocol : public Ipv4RoutingProtocol
{
public:
  friend class ::Split3MprTestCase;
  ...
  /**
   * \brief Set the SPLIT3 main address to the first address on the indicated interface.
   *
  ...
  /**
   * Return the list of routing table entries discovered by SPLIT3
   */
...
public:
   ...
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "Split3State::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
   ...
   *         to the SPLIT3 routing protocol. Entries from this associated
   *         routing table that use non-split3 outgoing interfaces are added
...
private:
  ...
  Split3State m_state;  //!< Internal state with all needed data structs.
  ...
  /**
   * Receive an SPLIT3 message.
   * \param socket The receiving socket.
   */
  void RecvSplit3 (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-SPLIT3 outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-SPLIT3 interface, false otherwise.
   */
  bool UsesNonSplit3OutgoingInterface (const Ipv4RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the SPLIT3 agent.
   *
  ...
  split3::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief SPLIT3's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param split3Message The %SPLIT3 message which must be forwarded.
   ...
   */
  void ForwardDefault (split3::MessageHeader split3Message,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);

  /**
   * \brief Enques an %SPLIT3 message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %SPLIT3 messages in
   * a same %SPLIT3 packet.
   *
   * \param message the %SPLIT3 message which must be sent.
   * \param delay maximum delay the %SPLIT3 message is going to be buffered.
   */
  void QueueMessage (const split3::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %SPLIT3 packets as needed in order to send all buffered
   * %SPLIT3 messages.
   *
   * Maximum number of messages which can be contained in an %SPLIT3 packet is
   * dictated by SPLIT3_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %SPLIT3 HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %SPLIT3 TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %SPLIT3 MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %SPLIT3 HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %SPLIT3 message which contains the HELLO message.
   ...
   */
  void ProcessHello (const split3::MessageHeader &msg,
   ...
   *
   * \param msg The %SPLIT3 message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const split3::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %SPLIT3 message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const split3::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %SPLIT3 message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const split3::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const split3::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const split3::MessageHeader &msg,
                            const split3::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const split3::MessageHeader &msg,
                                  const split3::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const split3::MessageHeader &msg,
                               const split3::MessageHeader::Hello &hello);
  ...
  // (reason: for SPLIT3 Link Sensing we need to know on which interface
