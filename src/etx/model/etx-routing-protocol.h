...
#ifndef ETX_AGENT_IMPL_H
#define ETX_AGENT_IMPL_H

#include "etx-header.h"
#include "ns3/test.h"
#include "etx-state.h"
#include "etx-repositories.h"
...
class EtxMprTestCase;

namespace ns3 {
namespace etx {

///
/// \defgroup etx ETX Routing
/// This section documents the API of the ns-3 ETX module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup etx
/// An %ETX's routing table entry.
...
/// \ingroup etx
///
/// \brief ETX routing protocol for IPv4
///
class RoutingProtocol : public Ipv4RoutingProtocol
{
public:
  friend class ::EtxMprTestCase;
  ...
  /**
   * \brief Set the ETX main address to the first address on the indicated interface.
   *
  ...
  /**
   * Return the list of routing table entries discovered by ETX
   */
...
public:
   ...
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "EtxState::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
   ...
   *         to the ETX routing protocol. Entries from this associated
   *         routing table that use non-etx outgoing interfaces are added
...
private:
  ...
  EtxState m_state;  //!< Internal state with all needed data structs.
  ...
  /**
   * Receive an ETX message.
   * \param socket The receiving socket.
   */
  void RecvEtx (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-ETX outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-ETX interface, false otherwise.
   */
  bool UsesNonEtxOutgoingInterface (const Ipv4RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the ETX agent.
   *
  ...
  etx::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief ETX's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param etxMessage The %ETX message which must be forwarded.
   ...
   */
  void ForwardDefault (etx::MessageHeader etxMessage,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);

  /**
   * \brief Enques an %ETX message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %ETX messages in
   * a same %ETX packet.
   *
   * \param message the %ETX message which must be sent.
   * \param delay maximum delay the %ETX message is going to be buffered.
   */
  void QueueMessage (const etx::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %ETX packets as needed in order to send all buffered
   * %ETX messages.
   *
   * Maximum number of messages which can be contained in an %ETX packet is
   * dictated by ETX_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %ETX HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %ETX TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %ETX MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %ETX HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %ETX message which contains the HELLO message.
   ...
   */
  void ProcessHello (const etx::MessageHeader &msg,
   ...
   *
   * \param msg The %ETX message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const etx::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %ETX message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const etx::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %ETX message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const etx::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const etx::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const etx::MessageHeader &msg,
                            const etx::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const etx::MessageHeader &msg,
                                  const etx::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const etx::MessageHeader &msg,
                               const etx::MessageHeader::Hello &hello);
  ...
  // (reason: for ETX Link Sensing we need to know on which interface
  ...
  Ptr<WifiNetDevice> GetWifiNetDevice(const Ipv4Address &addr);
