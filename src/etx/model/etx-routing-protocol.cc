...
///
/// \brief Implementation of ETX agent and related classes.
///
/// This is the main file of this software because %ETX's behaviour is
/// implemented here.
///
...
#include "etx-routing-protocol.h"
...
///
/// We only use this value in order to define ETX_NEIGHB_HOLD_TIME.
///
#define ETX_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define ETX_NEIGHB_HOLD_TIME   Time (3 * ETX_REFRESH_INTERVAL)
/// Top holding time.
#define ETX_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define ETX_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define ETX_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define ETX_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define ETX_UNSPEC_LINK        0
/// Asymmetric link type.
#define ETX_ASYM_LINK          1
/// Symmetric link type.
#define ETX_SYM_LINK           2
/// Lost link type.
#define ETX_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define ETX_NOT_NEIGH          0
/// Symmetric neighbor type.
#define ETX_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define ETX_MPR_NEIGH          2


/********** Willingness **********/

/// Willingness for forwarding packets from other nodes: never.
#define ETX_WILL_NEVER         0
/// Willingness for forwarding packets from other nodes: low.
#define ETX_WILL_LOW           1
/// Willingness for forwarding packets from other nodes: medium.
#define ETX_WILL_DEFAULT       3
/// Willingness for forwarding packets from other nodes: high.
#define ETX_WILL_HIGH          6
/// Willingness for forwarding packets from other nodes: always.
#define ETX_WILL_ALWAYS        7


/********** Miscellaneous constants **********/

/// Maximum allowed jitter.
#define ETX_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define ETX_MAX_SEQ_NUM        65535
/// Random number between [0-ETX_MAXJITTER] used to jitter ETX packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, ETX_MAXJITTER)))


#define ETX_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define ETX_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define ETX_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define ETX_MAX_ADDRS          64


// ------------------------------------- START
///Constants for  ETX Metrics
#define ETX_MAXIMUM_METRIC 65535
#define ETX_DEFAULT_METRIC 4096
#define ETX_UNDEFINED_LQ 0
#define ETX_ETX_SEQNO_RESTART_DETECTION 256

#define ETX_ETX_UPDATE_TIME   Time (7 * ETX_REFRESH_INTERVAL)
// ------------------------------------- END


namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("EtxRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::etx::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (ETX_WILL_DEFAULT),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (ETX_WILL_NEVER, "never",
                                    ETX_WILL_LOW, "low",
                                    ETX_WILL_DEFAULT, "default",
                                    ETX_WILL_HIGH, "high",
                                    ETX_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive ETX packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::etx::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send ETX packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace),
                     "ns3::etx::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The ETX routing table has changed.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_routingTableChanged),
                     "ns3::etx::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created etx::RoutingProtocol");
  ...
  m_packetSequenceNumber = ETX_MAX_SEQ_NUM;
  m_messageSequenceNumber = ETX_MAX_SEQ_NUM;
  m_ansn = ETX_MAX_SEQ_NUM;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting ETX on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunEtx = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), ETX_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvEtx,  this));
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() ETX socket");
        }
  ...
  if (canRunEtx)
    {
      ...
      NS_LOG_DEBUG ("ETX on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %ETX packet following \RFC{3626} specification.
void
RoutingProtocol::RecvEtx (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("ETX node " << m_mainAddress << " received a ETX packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == ETX_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  etx::PacketHeader etxPacketHeader;
  packet->RemoveHeader (etxPacketHeader);
  NS_ASSERT (etxPacketHeader.GetPacketLength () >= etxPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = etxPacketHeader.GetPacketLength () - etxPacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Etx Msg received with type "
  ...
  m_rxPacketTrace (etxPacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case etx::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s ETX node " << m_mainAddress
            ...
            case etx::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s ETX node " << m_mainAddress
        ...
        {
          NS_LOG_DEBUG ("ETX message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != etx::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all ETX messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == ETX_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == ETX_WILL_ALWAYS)
        {
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != ETX_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
...
void
RoutingProtocol::ProcessHello (const etx::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const etx::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for ETX Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for ETX Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
void
RoutingProtocol::ProcessTc (const etx::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const etx::MessageHeader::Tc &tc = msg.GetTc ();
...
//      const Ipv4Address &addr = *i;
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for ETX Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for ETX Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}

void
RoutingProtocol::ProcessMid (const etx::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const etx::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const etx::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const etx::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<etx::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (etx::MessageHeader etxMessage,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << etxMessage.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + ETX_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = etxMessage.GetOriginatorAddress ();
      newDup.sequenceNumber = etxMessage.GetMessageSequenceNumber ();
      newDup.expirationTime = now + ETX_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (ETX_DUP_HOLD_TIME,
...
void
RoutingProtocol::QueueMessage (const etx::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("ETX node " << m_mainAddress << " sending a ETX packet");

  // Add a header
  etx::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      i->first->SendTo (packet, 0, InetSocketAddress (bcast, ETX_PORT_NUMBER));
    }
}
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("Etx node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<etx::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  etx::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (ETX_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  etx::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<etx::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      etx::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("ETX HELLO message size: " << int (msg.GetSerializedSize ())
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (ETX_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  etx::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetVTime (ETX_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (ETX_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  etx::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<etx::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      etx::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-etx interface
          if (UsesNonEtxOutgoingInterface (route))
            {
  ...
  // add the routes using non-etx outgoing interfaces to the list
...
bool
RoutingProtocol::UsesNonEtxOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-ETX interface if a match is found
  // before reaching the end of the list of excluded interfaces
  return ci != m_interfaceExclusions.end ();
}

void
RoutingProtocol::LinkSensing (const etx::MessageHeader &msg,
                              const etx::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  ...
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Etx node " << m_mainAddress
  ...
  if (link_tuple == NULL)
    {
      ...
      newLinkTuple.RLQ = 1;
      newLinkTuple.DLQ = ETX_UNDEFINED_LQ;
      newLinkTuple.ETX = ETX_DEFAULT_METRIC;
      // ----------------------------------------------------------------------- END
  ...
  else
    {
      ...
      if (diff > ETX_ETX_SEQNO_RESTART_DETECTION)
      {
        diff = 1;
      }
  ...
  for (std::vector<etx::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      if ((lt == ETX_SYM_LINK && nt == ETX_NOT_NEIGH)
          || (nt != ETX_SYM_NEIGH && nt != ETX_MPR_NEIGH
              && nt != ETX_NOT_NEIGH))
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              ...
              else if (lt == ETX_SYM_LINK || lt == ETX_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + ETX_NEIGHB_HOLD_TIME;
  ...
  if (link_tuple->DLQ == ETX_UNDEFINED_LQ)
  {
    link_tuple->ETX = ETX_DEFAULT_METRIC;
  }
  else
  {
    link_tuple->ETX = link_tuple->RLQ * link_tuple->DLQ;
  }
  // --------------------------------------------------------------------------- END
  ...
  if (created)
    {
      ...
      m_events.Track (Simulator::Schedule (DELAY (std::min (link_tuple->time, link_tuple->symTime)),
                                           ...
                                           link_tuple->neighborIfaceAddr));
      // Schedule ETX timer
      m_events.Track (Simulator::Schedule (ETX_ETX_UPDATE_TIME, // -------------------------------- NEW
...
void
RoutingProtocol::PopulateNeighborSet (const etx::MessageHeader &msg,
                                      const etx::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const etx::MessageHeader &msg,
                                            const etx::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Etx node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<etx::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == ETX_SYM_NEIGH || neighborType == ETX_MPR_NEIGH)
                {
              ...
              else if (neighborType == ETX_NOT_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("Etx node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}

void
RoutingProtocol::PopulateMprSelectorSet (const etx::MessageHeader &msg,
                                         const etx::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
ETX::mac_failed (Ptr<Packet> p)
{
  ...
  ETX_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL)
    {
      link_tuple->lost_time () = now + ETX_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + ETX_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, ETX_WILL_DEFAULT);
...
//         Simulator::Now (),
//         ETX::node_id(ra_addr()),
//         ETX::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == ETX_STATUS_SYM) ? "sym" : "not_sym"));
...
//         ETX::node_id(ra_addr()),
//         ETX::node_id(tuple->neighborMainAddr),
//         ETX::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (ETX_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         ETX::node_id(ra_addr()),
//         ETX::node_id(tuple->dest_addr()),
//         ETX::node_id(tuple->last_addr()),
//         tuple->seq());
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (ETX_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber ()
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (ETX_MAX_SEQ_NUM + 1);
...
RoutingProtocol::EtxTimerExpire (Ipv4Address neighborIfaceAddr)
{
  ...
  m_events.Track (Simulator::Schedule (ETX_ETX_UPDATE_TIME,
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Etx node " << m_mainAddress
      ...
      if (numOifAddresses == 1)
        ...
        {
          /// \todo Implment IP aliasing and ETX
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and ETX");
        }
...
Ptr<WifiNetDevice>
RoutingProtocol::GetWifiNetDevice(const Ipv4Address &addr)
{
         ...
         m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
    {
        if (addr == i->second.GetLocal()) {
            Ptr<NetDevice> netDevice = i->first->GetBoundNetDevice();
            device = netDevice->GetObject<WifiNetDevice> ();
        }
    }
    return device;
}
