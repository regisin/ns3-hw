...
#define ETX_MSG_HEADER_SIZE 12
#define ETX_PKT_HEADER_SIZE 4
...
SecondsToEmf (double seconds)
{
  ...
  for (b = 0; (seconds / R_ETX_C) >= (1 << b); ++b)
    {
    }
  NS_ASSERT ((seconds / R_ETX_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds / R_ETX_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16 * (seconds / (R_ETX_C * (1 << b)) - 1);
...
///
/// \param etxFormat number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t etxFormat)
{
  int a = (etxFormat >> 4);
  int b = (etxFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return R_ETX_C * (1 + a / 16.0) * (1 << b);
}
...
{
  static TypeId tid = TypeId ("ns3::etx::PacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("Etx")
...
{
  static TypeId tid = TypeId ("ns3::etx::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("Etx")
...
{
  uint32_t size = ETX_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = ETX_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - ETX_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - ETX_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - ETX_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - ETX_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
